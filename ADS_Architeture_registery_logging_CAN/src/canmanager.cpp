// canmanager.h - Central CAN communication manager
#pragma once
...
bool CANManager::registerInterface(const QString& name, ICANInterface* interface)
{
    if (m_interfaces.contains(name)) {
        LOG_WARNING(LogCategory::CAN,
                    QString("Interface already registered: %1").arg(name));
        return false;
    }
    ...
    LOG_INFO(LogCategory::CAN,
             QString("Interface registered: %1 (%2)").arg(name).arg(interface->description()));

    return true;
}

ICANInterface* CANManager::getInterface(const QString& name)
{
    return m_interfaces.value(name, nullptr);
}

QStringList CANManager::availableInterfaces() const
{
    return m_interfaces.keys();
}

bool CANManager::setActiveInterface(const QString& name)
{
    ICANInterface* interface = m_interfaces.value(name, nullptr);

    if (!interface) {
        LOG_ERROR(LogCategory::CAN,
                  QString("Interface not found: %1").arg(name));
        return false;
    }

    // Disconnect old interface
    if (m_activeInterface) {
        disconnect(m_activeInterface, nullptr, this, nullptr);
    }

    m_activeInterface = interface;
    m_activeInterfaceName = name;

    // Connect new interface
    connect(m_activeInterface, &ICANInterface::messageReceived,
            this, &CANManager::onMessageReceived);

    connect(m_activeInterface, &ICANInterface::errorOccurred,
            this, &CANManager::errorOccurred);
