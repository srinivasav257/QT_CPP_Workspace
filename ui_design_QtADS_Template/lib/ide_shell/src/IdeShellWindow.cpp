...
namespace ide_shell
{
IdeShellWindow::IdeShellWindow(QWidget *parent)
    : QMainWindow(parent)
{
    buildShell();
    applyShellStyle();
    setWorkspaceWidget(createWelcomePanel());
}

IdeShellWindow::~IdeShellWindow() = default;

QWidget *IdeShellWindow::workspaceHost() const
{
    return m_workspaceHost;
}

void IdeShellWindow::setWorkspaceWidget(QWidget *widget)
{
    if (!m_workspaceHost)
        return;

    auto *layout = qobject_cast<QVBoxLayout *>(m_workspaceHost->layout());
    if (!layout)
    {
        layout = new QVBoxLayout(m_workspaceHost);
    ...
    if (widget)
    {
        widget->setParent(m_workspaceHost);
        layout->addWidget(widget);
    }
}

void IdeShellWindow::setStatusText(const QString &leftText, const QString &rightText)
{
    if (m_leftStatusLabel)
        m_leftStatusLabel->setText(leftText);
    if (m_rightStatusLabel)
        m_rightStatusLabel->setText(rightText);
}

QWidget *IdeShellWindow::createWelcomePanel() const
{
...
bool IdeShellWindow::eventFilter(QObject *watched, QEvent *event)
{
...
void IdeShellWindow::toggleMaximizeRestore()
{
...
void IdeShellWindow::buildShell()
{
    ...
    connect(m_maximizeButton, &QToolButton::clicked, this, &IdeShellWindow::toggleMaximizeRestore);
    ...
    m_workspaceHost = new QWidget();
    m_workspaceHost->setObjectName("editorArea");
    contentLayout->addWidget(m_workspaceHost);
    ...
    m_leftStatusLabel = new QLabel("<>   x 0   ! 0");
    m_leftStatusLabel->setObjectName("statusText");
    statusLayout->addWidget(m_leftStatusLabel);

    statusLayout->addStretch();

    m_rightStatusLabel = new QLabel("Autocomplete   |   Spaces: 4   |   UTF-8");
    m_rightStatusLabel->setObjectName("statusText");
    statusLayout->addWidget(m_rightStatusLabel);

    rootLayout->addWidget(statusBarWidget);
}

void IdeShellWindow::applyShellStyle()
{
    setStyleSheet(R"(
        QMainWindow {
            background: #d9d9df;
