...
void registerSamplePanels()
{
    ...
    // ===== Explorer =====
    reg.registerPanel({
        "project_explorer", "Project Explorer", "Explorer",
    ...
    reg.registerPanel({
        "file_browser", "File Browser", "Explorer",
        ads::LeftDockWidgetArea,
        [](QWidget* p) {
    ...
    reg.registerPanel({
        "class_view", "Class View", "Explorer",
        ads::LeftDockWidgetArea,
        [](QWidget* p) {
    ...
    // ===== Editor =====
    reg.registerPanel({
        "code_editor", "Code Editor", "Editor",
    ...
    reg.registerPanel({
        "text_editor", "Text Editor", "Editor",
        ads::CenterDockWidgetArea,
        [](QWidget* p) {
            auto* edit = new QTextEdit(p);
    ...
    reg.registerPanel({
        "hex_editor", "Hex Editor", "Editor",
        ads::CenterDockWidgetArea,
        [](QWidget* p) {
            return makeTablePanel(p,
                {"Offset", "00","01","02","03","04","05","06","07",
                 "08","09","0A","0B","0C","0D","0E","0F", "ASCII"}, 16);
        }
    });
    ...
    reg.registerPanel({
        ...
        [](QWidget* p) { return makeOutputPanel(p, "Application console output..."); }
    });

    reg.registerPanel({
        "build_output", "Build Output", "Output",
        ads::BottomDockWidgetArea,
        [](QWidget* p) { return makeOutputPanel(p, "Build output will appear here..."); }
    });

    reg.registerPanel({
        "debug_output", "Debug Output", "Output",
        ads::BottomDockWidgetArea,
        [](QWidget* p) { return makeOutputPanel(p, "Debug messages..."); }
    });

    reg.registerPanel({
        "log_viewer", "Log Viewer", "Output",
        ads::BottomDockWidgetArea,
        [](QWidget* p) { return makeOutputPanel(p, "Log entries..."); }
    });
    ...
    reg.registerPanel({
        "settings_panel", "Settings", "Properties",
        ads::RightDockWidgetArea,
        [](QWidget* p) {
    ...
    reg.registerPanel({
        "style_editor", "Style Editor", "Properties",
        ads::RightDockWidgetArea,
        [](QWidget* p) {
            auto* edit = new QTextEdit(p);
        ...
        [](QWidget* p) {
            return makeTablePanel(p,
                {"Address", "00","01","02","03","04","05","06","07",
                 "08","09","0A","0B","0C","0D","0E","0F"}, 16);
        }
    });

    reg.registerPanel({
        "registers", "Registers", "Debug",
        ads::RightDockWidgetArea,
        [](QWidget* p) {
    ...
    // ===== Tools =====
    reg.registerPanel({
        "search_results", "Search Results", "Tools",
        ads::BottomDockWidgetArea,
        [](QWidget* p) {
    ...
    reg.registerPanel({
        "bookmarks", "Bookmarks", "Tools",
        ads::BottomDockWidgetArea,
        [](QWidget* p) {
            return makeListPanel(p, {
                "main.cpp:10 - Entry point",
